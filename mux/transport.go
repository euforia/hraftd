...
func (mux *MuxedTransport) handleConn(conn net.Conn) {
	...
	if err := conn.SetReadDeadline(time.Now().Add(mux.Timeout)); err != nil {
		conn.Close()
		log.Errorf("tcp.Mux: cannot set read deadline: %s", err)
		return
	}
	...
	if _, err := io.ReadFull(conn, typ[:]); err != nil {
		conn.Close()
		log.Errorf("tcp.Mux: cannot read header byte: %s", err)
		return
	}
	...
	if err := conn.SetReadDeadline(time.Time{}); err != nil {
		conn.Close()
		log.Errorf("tcp.Mux: cannot reset set read deadline: %s", err)
		return
	}
	...
	if handler == nil {
		conn.Close()
		log.Errorf("tcp.Mux: handler not registered: %d", typ[0])
		return
	}
